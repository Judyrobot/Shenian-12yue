...
from ultralytics.nn.modules import (
    GhostV2,SimAM, ECA, SpatialGroupEnhance, TripletAttention, CoordAtt, GAMAttention,
    ShuffleAttention, SKAttention, DoubleAttention, CoTAttention, EffectiveSEModule,
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, (Detect, Detect_dyhead)):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            s = 256  # 2x min stride,这个注释掉了，恢复原状是s=256
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # ------------ 主干 ↑--------------
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Detect_dyhead, Pose, OBB, ImagePoolingAttn, v10Detect}
        ):
            args.append([ch[x] for x in f])
            if m is Segment or m is YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Detect_dyhead, Pose, OBB}:
                m.legacy = legacy
